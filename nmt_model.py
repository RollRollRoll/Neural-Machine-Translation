...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens = nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first=True,
                                                       total_length=source_padded.size(0))  # ,padding_value='<pad>')
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden, dec_cell = self.decoder(Ybar_t,
                                            dec_state)  # Ybar_t [b,2e] dec_state h[b,e],c[b,e]  dec_hidden [b,e] dec_cell [b,e]
